...
public class Editeur {
    ...
    public void setNotes(String notes) throws DataExceptions {
        Pattern p = Pattern.compile("[- '^~¨°²³±µ»«:-@!-/0-9À-ȳa-zA-Z]{0,5000}");
        Matcher m = p.matcher(notes);
        if (m.matches()) {
            this.notes = notes;
        } else {
            throw new DataExceptions(1005, "La description est limitée à 5000 caractères", "Desc");
        }
    }
...
//        else
    ...
    public void save(Xchange echange, String value) {
        ...
        try {
            ...
            if (this.ID == null) {
                ...
                query = "UPDATE EDITEUR "
                        + value + " WHERE PUBLISHERID = " + this.ID;
            }
    ...
    public String genValue() {
        ...
        if (this.ID == null) {
            ...
            str += " PUBLISHERPHONE = '" + tel + "', ";
            str += " PUBLISHERNOTES = '" + apostrophe(notes) + "'";
            if(editDate != null)
                str += ", STATEDITDATE = '" + editDate + "'";
