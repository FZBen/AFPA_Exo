...
public class Statut {
    ...
    public Statut() {
	    ...
	    public Statut(String ID, String statusname, String statusdescr) {
        this.ID = ID;
    ...
    public void setStatusdescr(String statusdescr) throws DataExceptions {
        Pattern p = Pattern.compile("[- '^~¨°²³±µ»«:-@!-/0-9À-ȳa-zA-Z]{0,1000}");
        Matcher m = p.matcher(statusdescr);
        if (m.matches()) {
            this.statusdescr = statusdescr;
        } else {
            throw new DataExceptions(17003, "La description est limitée à 1000 caractères", "Desc");
        }
    }
